...
保存不可随意改写的博主原始稿。除明确标记的转写修正外，raw 文件不在 ingest 后改写。
...
5. `lint_knowledge.py` 能在仓库上通过。
6. GLM vendored 来源、许可证和本地 patch 已记录。
...
4. 首版不要求迁移全部历史 Raw，可先使用精选 fixture 和后续增量导入。
5. 系统目标是学习和分析，不是自动交易。
...
- 首个 implementation plan 优先做仓库基础、schema、`qing-learning` 和 GLM vendor。
- 脚本只辅助，不替代 LLM 的语义学习。
