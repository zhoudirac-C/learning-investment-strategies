...
## 资讯列表分页实测（2026-08-16，结论：不可得）

对 `IndexPlate.GetIndexList` 的分页参数实测全部无效——`Page/page/PageIndex/pageNo/`
`LastID/last_id/MinID/MaxID/ID/LastCreateTime/CreateTime/Time` 及 `st=0..4` 均返回
同一"最新窗口"（8 条，跨当日+次日），服务端忽略全部分页/游标参数。真正的翻页
疑走原生证书固定通道（与搜索同命，见「搜索/文章接口抓包」节）。
**后果与处置**：日更只拿最新窗口，若某日资讯超出一页可能漏收；实测日产量 ≈ 窗口大小
（08-12~08-14 为 3/9/7 条），漏收风险低但存在。如需更强保障可每日两拉（午+收），
当前维持 17:45 单拉 + 如实记录。
