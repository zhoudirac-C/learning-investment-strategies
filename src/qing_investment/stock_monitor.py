...
def format_agent_analysis_context(
    ...
    lines.extend(
        [
            ...
            "格式固定，最多350字，禁止Markdown表格、分级标题、长篇数据罗列和研报式分析：",
            "【盘面】一句话定性。",
            "【持仓池】每个触发/重点持仓一行：标的：动作=持有/做T/减仓观察/风控观察；触发=...；证伪=...",
            "【观察池】可买=列最多3个满足/接近买入条件的标的和买点；暂不买=一句话说明主因。",
            "【脚注】只写数据源、时间戳、行情异常。不要给无条件买卖指令。",
        ]
    )
    return "\n".join(lines)
...
def format_live_analysis_context(config: MonitorConfig, value: datetime) -> str:
    ...
    lines.extend(
        [
            ...
            "最多350字，禁止Markdown表格、分级标题、长篇数据罗列和研报式分析。",
            "格式：【盘面】一句话；【持仓池】动作+触发+证伪；【观察池】可买/暂不买+买点；【脚注】数据源/时间/异常。",
            "不要给无条件买卖指令。",
        ]
    )
    return "\n".join(lines)
