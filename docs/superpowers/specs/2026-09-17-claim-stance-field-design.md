...
| **`view`** | UP 自己的判断 | 看好/看空、操作建议、选股判据、对未来的预判、方法论文 |
...
| # | 事项 | 建议 |
|---|---|---|
| 1 | 字段名用 `stance` 还是 `nature` / `epistemic_type` | `stance`（最短、语义清晰） |
| 2 | 三值 `fact/view/mixed` 是否够 | 够；`mixed` 覆盖"先陈述后判断" |
| 3 | 是否**同步实施**图库层（Neo4j 属性 + Qdrant payload） | 建议**同步**，否则查询侧用不上 |
| 4 | 是否回填存量 5,234 条 | 建议**暂不回填**，新 claim 生效即可 |
| 5 | 是否改 `discover` 的 fact 降权 | 建议**先不动**，观察轮后再评估 |
| 6 | **是否同步改 Step 1 提取门槛**（§六） | 建议**必须做**——只加字段等于给垃圾贴标签 |
| 7 | 门槛需否保留一份"事实表" | 建议：**完全跳过**，事实只在 raw/wiki 留痕；不进 claim 库 |
