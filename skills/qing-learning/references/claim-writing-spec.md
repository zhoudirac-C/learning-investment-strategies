...
### 0.0 措辞精度——"正在挖" vs "已经处于"

**问题**：LLM 倾向将"进行中的过程"简化为"已确认的状态"，丢失关键的阶段信息。

**反面案例（2026-06-07）**：
- UP 原文："目前是一个挖黄金坑的时候"（正在进行时）
- AI 提取：`subject: 当前市场阶段——黄金坑` / `statement: 当前是"黄金坑"阶段`（完成时）
- 用户纠正："当前市场阶段是正在挖黄金坑，不是已经处于黄金坑"

**规则**：
- 当 UP 使用「正在」「还在」「还没到」等进行时/否定表述时，**claim 的 subject 和 statement 必须保留进行时态**
- 不要将「正在挖坑」简化为「处于坑中」——前者意味着坑底未现、需等信号确认，后者暗示可以抄底
- **检查方法**：对照 evidence_quote 中的时态关键词（正在/还没/先/等/如果），确保 claim 不失原意

**修正后**：`subject: 当前市场阶段——正在挖黄金坑，尚未到底` / `statement: 当前处于"正在挖黄金坑"的阶段...坑还没挖完——需等CPI数据、原油走向、信用利差等变量确认坑的深度。`

**影响范围**：所有含「进行时」「条件句」「未完成」表述的 claim，尤其 market-cycle 和 macro 类型。
