...
### 设计演进：为什么从"单文件独立模式"走向"通用框架+examples"

**用户原始疑问**（典型信号）："这里面的推理思路都是从单个文件提取的，这样是否不够通用？比如只能针对一个方向投资的思路？"

**问题诊断**：
- Phase 5 及之前：每篇含推理链的 raw 生成一个独立 `pattern`，116 个模式中 99.1% 只关联 1 个 raw
- 后果：①文件持续膨胀 ②主题高度重叠 ③匹配噪声增大 ④无法跨 raw 复用推理结构

**核心洞察**：UP 的推理结构是**可复用的**——"上游涨价周期分析"这个推理框架，既适用于 MLCC，也适用于 PCB、铜箔、存储。差异只在具体标的和证据，推理骨架相同。

**Phase 6 解法**：
1. **预定义 10 个通用框架**（如 `upstream_cycle`、`mainline_identification`）——代表 UP 反复使用的推理骨架
2. **提取时 LLM 判断归入哪个框架**——不是"新增一个模式"，而是"这是 `upstream_cycle` 的一个新 example"
3. **框架的 `examples` 列表积累**——同一推理结构在不同 raw 中的具体应用

**类比**：
- Phase 5 = 每道菜写一本菜谱（116 本，每本只做一次）
- Phase 6 = 10 本通用菜谱（炒、炖、蒸...），每道菜作为具体案例归入对应菜谱

**何时该用独立 pattern**：
- 当 LLM 无法归入任何通用框架时（`matched_framework` 为空或 `others`）
- 当发现新的、跨 raw 反复出现的推理结构时 → 考虑新增为第 11 个通用框架
- 独立 pattern 增长应接近 0，若持续增长说明通用框架覆盖不足
