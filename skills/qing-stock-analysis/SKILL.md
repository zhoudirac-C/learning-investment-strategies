...
【观察池】
- 📋 条件单待触发：最多3个接近买入条件的标的和触发条件（价格区间+确认信号）。无满足的写"暂无可买"。
- ❌ 不买：最多3个不符合条件或风险较大的标的和原因。（如"大盘未止跌企稳，全部进攻组跌幅>1.5%"）。
...
【观察池】
- 📋 条件单待触发：最多3个接近买入条件的标的和触发条件（价格区间+确认信号）。无满足的写"暂无可买"。
- ❌ 不买：最多3个不符合条件或风险较大的标的和原因。（如"上证失守4000+进攻方向集体走弱，不满足任何买入条件"）。
...
## 子任务：持仓方向与 UP 近期内容核对（Direction Alignment Check）

当用户问"UP 是否提到我的持仓""核对我的持仓方向""我的持仓方向 UP 怎么看"或类似表述时，触发本流程。

### 与"持仓更新"的区别

| 维度 | 持仓更新 | 方向核对 |
|------|---------|---------|
| 核心目标 | 盈亏 + 技术面 + Claims | UP 近期是否提及 + 态度 |
| 数据重点 | 实时行情、K线 | UP 内容（视频/复盘/早盘/动态） |
| 输出重点 | 操作建议 | 方向一致性评估 |
...
在给出核对结果前，必须读取**近期所有 UP 内容**（早盘、复盘专栏、视频录音、动态），按语言强度进行方向优先级交叉排序。
...
3. **交叉比较**：若多个 source 对不同方向的评级不一致，使用语言强度更高的那个作为该方向的真实定级。例如：6-5 早盘说 AI 上游材料"最确定"(🔥🔥)，6-4 视频说燃气轮机"类比锂电池"(🔥🔥🔥) → 燃气轮机 > AI 上游材料。
4. **汇总输出**方向优先级排名表，包含每个方向的 source 引用和语言强度引用。
5. 基于此排名表确定方向一致性评估，再进入逐只核对。

1. **读取持仓**：从 `positions.yaml` 提取 `shares > 0` 的标的，按方向分组
2. **扫描 UP 内容**：读取 `sources/raw/财经/` 最近 2-3 天的所有内容（视频/复盘/早盘/动态）
3. **逐只提取**：grep 搜索每只标的是否被提及，记录来源、原话、语言强度
4. **语言强度评级**：🔥🔥🔥最高 / 🔥🔥高 / 🔥中 / ⚠️低 / ❌未提及
5. **交叉比较**：同一方向多个 source 评级不一致时，取语言强度最高的
6. **输出核对表**：标的 | 是否提到 | 来源 | UP原话 | 语言强度

### 关键约束

- **必须检查所有媒介类型**：不能只读复盘不看视频（视频通常更详细）
- **必须检查评论回复**：UP 动态下的评论可能包含与主动态相反的条件
- **区分方向与个股**：UP 看好"燃气轮机"方向 ≠ 点名"万泽股份"
- **数据完整性检查**：若最新文件超过 2 天，提示用户可能遗漏近期更新
- **不做盈亏计算**：本流程不做 P&L 和技术面分析，只做方向一致性评估
...
### 陷阱 1: 简单阈值框架误导短期预判（2026-06-11 用户纠正）

**反面案例**：分析雅克科技(002409)2板+烂板+18%换手后，问\"明天是否危险\"。AI 回答了一个以\"低开>3%=出货\"为核心的简化参考表，被用户纠正。

**纠正要点**：
- **低开 ≠ 出货**：连续2板后第二天低开，更大概率是多空分歧的自然延续。把低开等价于主力出货是过度简化的归因。
- **任意阈值缺少校准依据**：\"3%\"这个阈值从哪里来的？它是134.81的3%，但当天开盘才126.22、最低124.00——130.77还在当日波动范围内，不是恐慌性低开信号。
- **更好的框架是竞价+前15分钟组合，不是单个价格阈值**：

| 竞价+前15分钟 | 含义 |
|--------------|------|
| 大幅低开（竞价<-5%）+ 无承接 | 恐慌性出逃 |
| 小幅低开（-1~-3%）+ 缩量企稳 | 正常获利了结，可修复 |
| 竞价从跌停拉回平盘 | 资金抢筹，强者恒强 |
| 平开/高开+放量冲板 | 接力成功，但追高风险大 |

- **更关键的微观指标**：竞价成交量（>昨日5%且持续走低→竞价出货）、前15分钟量能（缩量企稳=健康，放量下跌=危险）、同板块联动（多只同步走弱→板块性问题）

**教训**：
1. **不要用单维度阈值替代组合判断**：低开3%=出货是线性思维，市场不是线性的
2. **阈值必须注明来源**和校准依据；说不出来源就用条件组合（竞价+量能+联动）
3. **涨停/连板股的分析框架是竞价+量能+板块联动**，不是价格幅度
4. **分析目标是列出可验证条件**，不是预测方向——\"如果竞价缩量企稳+板块不弱→有修复可能\" 比 \"明天危险\" 有用一万倍
5. **不要给出简化的结论式操作参考表**——低开→Y、高开→Z 看似实用，实际是用单一变量概括多变量市场
...
23. **竞价+前15分钟分析框架（2026-06-11 用户框架）**：连续涨停/烂板/趋势股的分析不能用单维度阈值（如"低开>3%=出货"），必须用竞价+前15分钟组合判断。详见 `docs/design/individual-stock-deep-analysis-design.md` §8.1（持仓类型分支框架）和本 skill 陷阱 1。

24. **腾讯API竞价数据获取方式（2026-06-11 验证）**：
    - 竞价价格 = 腾讯API `今开` 字段 (a[6]) ✅ 直接可用
    - 竞价量 = 不可精确分离（附在 09:30 首根5K线内），用 `首根量/近5日均值` 近似
    - 9:20-9:25 轨迹 = 无 API 支持，标记为 `null`
    - 量比 = `a[50]` ✅ 直接可用
    - 东财竞价/明细API = 返回 null，不可用
...
26. **Agent 输出标签必须与正文结论一致（2026-06-11 新发现）**：「✅ 可买」这个标签让用户以为可以立即买入，但正文写的是「等缩量企稳信号」。标题和正文的语义矛盾导致用户误判。规则：**Agent 输出中禁止出现「可买」标签**，改用「📋 条件单待触发」。如果条件未全部满足，永远不要写「可买」「建议买入」等确定性表述。详见 `qing-stock-monitor-update` 陷阱 24。
