...
### 陷阱 3c: 观察池标的股价大幅偏离 entry_zone 后静默错误触发

**反面案例（2026-06-12）**：圣泉集团(605589)连续大涨至 ~59 元，原 entry_zone 为 50-53 元。`add_zone` 检查用 `zone[0] <= latest <= zone[1]` 做范围判断——price=59 明显不在 50-53 区间内，检查本身通过（不触发）。但 6 条件评估中的`buy_signal` 检测使用了另一种路径，价格远超区间上限却仍返回 `true` 并写入 `buy_signals`——因为它只检测 price 是否在区间内，没有考虑**price 已经远高于区间上限时的语义错误**：price > zone_upper 应该是「价格跑远了，不触发」，而不是「价格不在区间里，刚好不触发」。

**已修复**：`stock_monitor.py` 新增价格偏离度保护：`latest > zone_upper × 1.05` 时强制判定为 `"偏离，不触发"`（日志记录 `buy_signal_deviation`）。

**观察池生命周期管理规则**：

当标的股价持续上涨，远离原有 entry_zone 时：

| 标的角色 | 原区间状态 | 建议动作 | 原因 |
|---------|-----------|---------|------|
| P1/P2 核心 | 区间失效但逻辑未变 | 移除 entry_zone 或标记 `lifecycle.stage: paused` | 等回踩重置区间，不删除标的本身 |
| P3 观察 | 区间失效 | 直接设置 `entry_zone.price_range: null` | P3 本就不设介入区间 |
| 所有角色 | 连续涨停 | 临时移除候选，wait_reason 注明"等分歧回踩" | 涨停期间不追高，等分歧日重新计算区间 |

**不要在标的涨上去后删除它**。保留在 theme 中但停用 `entry_zone`，等回踩后重新计算介入价位再恢复。
