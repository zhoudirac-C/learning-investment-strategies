...
# ⚠️ 2026-09-10：IDX880823（TDX 微盘股）因 TDX 接口封禁失效，数据源改为
# 同花顺 883418（市值最小400只等权，与万得同口径）。别名保留 IDX880823
# 以免破坏 blindtest pack / 既有评测基线口径，但底层 code 指向新源。
# ⚠️ IDX000932 实为「中证消费」（原注释误标中证2000，中证2000 应为 sh932000），
# 此处保留映射不变（消费指数本身有效），仅修正注释。
INDEX_ALIAS_TO_CODE = {
    ...
    "IDX000932": "sh000932",  # 中证消费（非中证2000！）
    "IDX880823": "883418",    # 微盘股指数（同花顺，原 TDX 880823 已失效）
}
