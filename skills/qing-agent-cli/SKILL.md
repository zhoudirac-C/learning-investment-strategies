...
## 核心设计原则：知识库是方法论，不是信息来源

这是 Qing-Agent 最重要的设计约束，必须严格遵守：

### 知识库的正确角色

| 角色 | 正确用法 | 错误用法 |
|------|---------|---------|
| **方法论指南** | 学习UP如何分析（框架、步骤、概念定义） | 直接引用UP历史观点作为当前判断依据 |
| **思维模型** | 参考"冰点期""劣性轮动"等概念的定义 | 把UP三天前的"看多半导体"当作今天看涨的理由 |
| **风格参考** | 模仿UP的表达方式和语气 | 复述UP的具体持仓建议而不验证 |

### 为什么这个区分至关重要

1. **信息时效性**：claims是历史观点，市场每天都在变。昨天UP看多，今天可能已清仓
2. **独立判断**：AI必须基于实时数据独立分析，不能当"复读机"
3. **用户信任**：如果AI照搬UP错误观点（如6月5日UP喊多但市场冲高回落），会损害用户利益

### Prompt设计规范

所有涉及知识库的prompt必须明确区分：

```
【输入数据说明】
- claims: 博主历史观点（仅作参考，不得作为当前判断依据）
- framework_rules: 分析方法论框架（指导如何思考）
- external_sector_boards: 外部行情源的真实数据（主要依据）
- market_snapshot: 实时行情快照（主要依据）

【使用规则】
- 所有判断必须基于实时数据
- 不得引用claim ID作为当前判断的依据
- 区分"方法论概念"（可以引用）和"具体观点"（必须验证）
- 如果实时数据与历史观点矛盾，以数据为准
```
