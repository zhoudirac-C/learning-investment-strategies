...
28. 价格结构前置否决（宽度指标无权单独定「企稳」）：预判今日 nature=「缩量企稳」或 market_stage=「震荡」前，必须先以 index 块收盘价序列做价格结构校验——(a) 破位校验：上证指数/创业板指昨日收盘跌破 5 日均线或近期波段低点（收盘价口径）且昨日收跌时，无论昨日涨跌家数/涨停家数等宽度指标多强，今日 nature 禁止预判「缩量企稳」——破位收跌后的宽度修复只按反抽处理（消耗调整时间，不构成企稳），今日 market_stage 优先预判「调整」；若指数破位但昨日收涨（修复尝试中），先做适用前置校验——昨日量能环比 ≥ 前日且涨跌主因非单一隔夜外部链映射（费半/存储链/KOSPI 等外力映射日）；任一不满足时该信号改标「反抽」，今日 stage 预判维持原判、不得据此改判「震荡」，收复条件写入 watch_next；前置均满足时仍判「震荡」须在 stage_reason 写明破位事实与收复条件（收回 5 日均线/波段低点上方）。(b) 顶部结构结论级压制：structure 块含任一指数 60min 及以上顶部 forming/divergence 信号时，今日 market_stage 禁止预判「主升」，且 stage_reason 必须说明该顶部结构对预判的压制（规则17 只要求引用，本条要求影响结论）；顶部结构叠加破位或 cycle_state.rebound_day 达到/超过 theoretical_window 上限时，今日优先预判「调整」，禁止把顶部结构只写进 watch_next/cycle_state.note 而维持原结论。
...
38. 阶段重定性冲突裁决+T+1确认：今日预判拟推翻昨日收盘 stage（尤其昨日「调整」改判今日「震荡」）时强制执行——①冲突裁决：昨日单日修复信号（收涨/宽度回升/未破位）与 ≥2 项在场调整侧证据（宏观利率压制未解除/60min 及以上顶部结构 forming/divergence 在位/量能源头=存量高低切非增量/宽基未收复 5 日线/规则32 防御轮动末端读数）冲突时，禁止即时翻转，昨日信号按「反抽观察」处理（沿用「缩量反抽（下跌中继）」口径），今日 stage 维持原判；②T+1确认：阶段改判须今日盘面验证（回踩不破关键均线+量能不缩）后方可生效，验证条件写入 watch_next；③stage-nature 一致性自检：nature 预判「主动降速」或 stage_reason 自述含调整成分时，market_stage 必须联动为「调整」，禁止「stage 震荡+nature 主动降速」脱钩输出；量能源头判定为存量高低切时，同等禁止「放量攻击」与「修复/阶段升级」两种定性（volume_source_qualify 对称约束）。"""
