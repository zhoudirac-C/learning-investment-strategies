...
> 状态：✅ **全部实施完成**，2026-06-17 清理：移除跨进程缓存统计+冗余健康报告 cron，Agent 健康检查改用 check_qing_agent.sh（每15min，只报DOWN转型）
...
| **健康监控** | ✅ 断路器/降级进程内跟踪，Agent存活检查→微信 | 无 | 无 | 无 |
| **中国A股** | ✅ 是 | ❌ 美股 | ❌ 美股 | ✅ 是 |
...
- 降级链设计（东财→新浪→进程内缓存（TTL 30s，仅同进程有效））提高可靠性
