...
### P0：压缩 Prompt 上下文（工程层）

**问题**：market_summary/stock_scanner prompt 超过 64KB 被截断，关键上下文可能丢失。

**建议**：
1. **分级加载 watchlist**：只把 P1-核心 和持仓股放入 LLM 上下文，P2/P3 用摘要而非完整条目。
2. **claims 按时间衰减**：只加载最近 7 天且与当前 query 相关的 claims，避免把半年历史全塞进 prompt。
3. **framework 按需加载**：当前 `_load_framework_files` 加载 4 个文件并截断到 4000 字符，应改为按 query 关键词匹配加载最相关的 1-2 个文件。
4. **sector_context 去重**：多个节点重复查询 Qdrant/Neo4j，应在 graph 开始时统一检索一次，后续节点复用。
...
### P1：建立"隔夜信息"预输入机制（数据层）

**问题**：UP 早盘大量依赖隔夜外盘、产业新闻，Agent 实时搜索不稳定。

**建议**：
1. 增加一个 **08:30 盘前准备节点**，提前运行一次外部信息聚合（美股收盘、日韩盘前、期货、重要产业新闻）。
2. 把聚合结果写入 `daily_state.pre_market_brief`，供 09:26/09:45 节点读取，避免每个节点都联网搜索。
3. 对外部搜索设置 fallback：若超时，则只使用已入库 claims/wiki，并在输出中标注"外部数据不可用，以下分析基于本地知识库"。
...
4. 压缩 market_summary/stock_scanner prompt：分级加载 watchlist、claims 按时间衰减、framework 按需加载。
...
| prompt 截断 | market_summary/stock_scanner prompt 稳定在 50KB 以下 |
