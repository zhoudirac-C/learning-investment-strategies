...
13a. **⚠️ 字段完整性强制检查**：写入 claims YAML 后，必须运行以下验证确保所有 18 个必需字段完整（参考 `references/claim-writing-spec.md` §〇）：
    ...
    ```
    **这是硬性门禁——不通过不能提交。**
13b. **⚠️ 股票代码完整性强制检查**（2026-06-08 新增，2026-06-09 补充）：字段检查通过后，必须逐条检查 claim 中涉及的个股是否已标注代码：
    - 检查 `statement` 中是否所有公司名称都带 6 位数字代码（格式：`公司名(6位代码)`）
    - 检查 `related_stocks` 是否已填写（涉及个股的 claim 必须填）
    - **`related_stocks: []` 必须显式填写**：即使 claim 涉及的公司全部未上市/不是A股，也必须写 `related_stocks: []` 标记"已检查、无相关标的"。完全省略 = Agent 无法区分"已检查无标的"和"忘记填了"。
    - **`related_stocks` 必须用结构化对象格式**：每个条目必须包含 `code`/`name`/`role` 三个字段。不能使用旧格式的简单字符串（如 `- 超捷股份(301123)`）。
      ```yaml
      # 正确格式
      related_stocks:
      - code: 600118.SH
      ...
      # 错误格式（旧格式，Agent 无法结构化检索）
      related_stocks:
      - 中国卫星(600118)
      ```
    - **`role` 字段标注可交易性**：对于非主板标的，在 `role` 中标注不可交易原因（如 `创业板不可交易` / `科创板不可交易`）。
    - 检查 `interpretation` 是否已标注 688/300 不可交易（如适用）
    - **不要以"先写claim后补代码"为借口**——skill规范要求"新写的claim应直接带代码"
    - **反面案例 2026-06-09**：claim-20260609-004 (a-h) 8条全部缺少 `related_stocks`。提取时认为"公司未上市=不需填"，审核时18必需字段检查不包含 `related_stocks`，双重过滤全漏。用户纠正后补了8条+修复001-e旧格式。
...
24. **⚠️ claim 原子性——禁止多主题/多标的合并**（2026-06-08）：一条 claim 只能包含一个主题、一个方向、或一只标的。反面案例：把化工+养殖+非银+光伏 4 个行业合并为一条 → Qdrant 语义向量被稀释，Agent 无法单独引用某个行业。把金禄电子+天准科技 2 只标的合并为一条 → 两只独立标的混在同一 statement。**纠正**：拆分为独立 claim，每条的 subject 是单一实体。**验证**：审核时检查 subject 是否含「/」「、」「+」等多实体标记。

25. **⚠️ 主题页 wiki 覆盖——创建专题页前必须检查已有文件（2026-06-09）**：创建 `knowledge/wiki/市场分析/` 下的主题页前，必须先用 `ls` 或 `git show HEAD:path` 检查是否存在。`write_file` 会静默覆盖。反面案例：`商业航天.md` 已在 6/7 创建，6/9 用 `write_file` 直接覆盖 → 旧 claims 引用丢失。**正确做法**：若文件已存在，在原有结构上追加/合并；`knowledge/wiki/log.md` 记录为"合并更新"而非"新增"。该原则同样适用于 `方法论/`、`知识图谱/` 等非每日复盘类页面。
