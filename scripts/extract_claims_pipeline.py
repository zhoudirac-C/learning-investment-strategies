...
7. **提取门槛（最重要，先于字段格式）**：只提取「该 UP 表达的判断/观点/方法论」，
   **单纯的数据和论据不算观点，一律不提**。

   ❌ **不要提**（这些是事实/论据，换谁写都成立）：
   - 行情数据：指数点位、涨跌幅、成交额、涨跌家数、涨停/跌停家数
   - 涨幅榜描述：「某板块领涨」「某股涨停」「某概念涨 X%」
   - 政策/文件转述：「X 部门印发《Y》」
   - 研报数据引用：「据 X 证券，某数据为 Y」
   - 公司资料罗列：主营业务、产能、客户、市占率
   - 客观事件播报：「某会议召开」「某公司公告」

   ✅ **要提**（只有该 UP 会这么说）：
   - 看多/看空判断、操作建议、仓位建议
   - 选股判据、方法论、对未来的预判
   - **对事实的解读**：出现「这说明…」「意味着…」「因此…」「关键看…」等
     推论语句时，提取**解读部分**，不要提取被解读的事实本身

   ⚠️ **判别法（逐条自问）**：
   - 「这句话换一个作者还成立吗？」
     · 成立 → 是事实/论据 → **不提**
     · 只有该 UP 会这么说 → 是观点 → **提**
   - 「这句话是在告诉我发生了什么，还是告诉我该怎么想/怎么做？」
     · 前者 → **不提**；后者 → **提**

   ⚠️ **混合句处理**：若一句话既含事实又含判断（「某数据 X，所以我认为 Y」），
   只提取**判断部分 Y**；事实 X 放进该 claim 的 `evidence_quote` 作为论据，
   不要为 X 单独建一条 claim。

   ⚠️ **框架 vs 个案（防止个案泄漏低复用条目）**：
   若某段罗列多个公司资料，每个公司末尾都跟一句「处于 X 阶段 / 需跟踪 Y 进度」，
   这类**个股层时点判断**与总括框架重复 → **只提总括框架那条，个股层不单独建 claim**，
   公司事实放进框架条的 `evidence_quote` 或 `related_stocks`。

   ⚠️ **复盘复核句不提**：含「按预期 / 按预判 / 如昨日所说」的段落，若只是核对昨天
   的判断有没有兑现、且**未产生新的方法论或新判断** → **不提**。
   （判据：这句话给未来提供了什么可复用的东西？没有 → 不提）
