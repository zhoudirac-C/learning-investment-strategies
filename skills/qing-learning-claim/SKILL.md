...
```
Step 1: Agent 读 raw → 【提取门槛过滤】→ 写宽松 JSON → Gate 1 校验 19 字段
...
### ⚠️ 提取门槛（最优先，2026-09-17 新增）

**只提「该 UP 表达的判断/观点/方法论」。单纯的数据和论据不算观点，一律不提。**

| ❌ 不提（事实/论据，换谁写都成立） | ✅ 要提（只有该 UP 会这么说） |
|---|---|
| 行情数据：点位、涨跌幅、成交额、涨跌家数、涨停/跌停家数 | 看多/看空判断、操作建议、仓位建议 |
| 涨幅榜描述：「某板块领涨」「某股涨停」「某概念涨 X%」 | 选股判据、方法论、对未来的预判 |
| 政策/文件转述：「X 部门印发《Y》」 | **对事实的解读**（「这说明…」「意味着…」「关键看…」） |
| 研报数据引用：「据 X 证券，某数据为 Y」 | |
| 公司资料：主营、产能、客户、市占率 | |
| 客观事件播报：会议召开、公司公告 | |

**逐条自问两个判别法**：
1. 「这句话换一个作者还成立吗？」成立 → 事实 → **不提**；只有该 UP 会这么说 → 观点 → **提**
2. 「这句话告诉我发生了什么，还是该怎么想/怎么做？」前者 → **不提**；后者 → **提**

**混合句**（「某数据 X，所以我认为 Y」）→ 只提判断部分 Y；事实 X 放进 `evidence_quote` 作论据，**不为 X 单独建 claim**。

**另外三个边界（2026-09-17 实测验证后补）**：
- **框架 vs 个案**：某段罗列多个公司资料、每家末尾跟「处于 X 阶段 / 需跟踪 Y」时，
  只提总括框架那条，**个股层不单独建 claim**（公司事实进 `evidence_quote` / `related_stocks`）
- **复盘复核句不提**：含「按预期/按预判/如昨日所说」的段落，若只是核对昨日判断是否兑现、
  **未产生新方法论或新判断** → 不提。（判据：给了未来什么可复用的东西？没有 → 不提）
- **弱应提边缘条**：与已有方法论条重叠度高的（如“亏钱效应收敛比预想快”→“情绪修复”条），
  倾向**并入**而非单列，避免把同一方法论切碎

**产出量参考**：一篇复盘 **8-15 条真观点**优于 30 条含大量播报。某段全是数据罗列、读后无立场 → **该段不产生 claim**。

> 📌 背景：此门槛因 2026-09-16 实测——旧规则从当晚复盘提了 38 条，其中 **30 条（79%）为纯事实陈述**，真正观点仅 5-8 条，导致「UP 看好什么」查询被行情播报污染。
> 📊 验证（2026-09-17 用新规则重跑同一篇）：**38 条 → 15 条**，剔除 23 条纯事实/论据（60%）；
> 校准点名的 5 条真观点 **5/5 全部命中**，另额外捞回 10 条此前未识别的真观点 —— 既堵住漏提，也未过度收紧。

### `stance` 字段（话语性质，2026-09-17 新增，四值，非必填）

每条 claim 应标注 `stance`，回答「这是**什么性质的话**」：
...
| `mixed` | 事实与判断兼有且**不可拆分** | 优先拆分，不轻易用 |

**`view` vs `market-regime` 判别（易混，看判断对象）**：
- 对象是**大盘/全局/风格/情绪周期** → `market-regime`
  · 「风格切换已经完成，但级别仍是反弹而非反转」
  · 「本轮情绪周期已进入尾部释放阶段」
  · 「用混沌期描述有直观意义，但不足以确认退潮结束」
- 对象是**某个板块/个股/主题** → `view`
  · 「依然看好国产替代方向」
  · 「电网设备属于跟随方向，独立行情需等招标数据」

**🔴 「市场处于什么阶段」一律算观点（market-regime），不算事实**——即使句子里带数据，
只要落点是**阶段/级别/性质定位**就是观点；数据部分放进 `evidence_quote`。

> 判据同门槛：把「成交额 1.84 万亿」换个作者说还成立 → 事实；
> 把「已进入尾部释放阶段」换个作者说不成立 → 观点。

**与 `claim_type` 的关系（正交）**：`claim_type` = 讲的是什么（主题）；
`stance` = 怎么讲的（话语性质）。例：
- `market-cycle` + `market-regime` → 「风格切换完成但仍是反弹」
- `market-cycle` + `fact` → 「指数收报 3891.60」
- `sector-theme` + `view` → 「电网设备为跟随方向」

**下游容错**：存量 5,234 条无 `stance`，查询时必须容错：
```python
# ✅ 缺字段视为 unknown，保留可见
[c for c in claims if c.get("stance", "unknown") in ("view", "market-regime")]
# ❌ 会丢掉全部存量
[c for c in claims if c.get("stance") == "view"]
```
...
```
Step 1:
☐ 【门槛】已过滤纯事实/论据（行情播报、涨幅榜、政策转述、研报引用、公司资料）——判别法：换个作者还成立吗？
☐ 已逐段阅读全文，无遗漏
☐ 每条 claim 只有 1 个主题
☐ 19 个必需字段齐全（含 up_id：B站 uid / `unknown` / `chanlun-original`）
☐ `stance` 已标（四值：fact / view / market-regime / mixed）——**阶段判定归 market-regime**
☐ 颗粒度检查：原文中每一个独立的**定性判断**（如"已到历史尾部区域"）都要提取为独立 claim，不要藏在同段另一条 claim 的 interpretation 中。
  ⚠️ 但**定量事实本身不提**（"23个交易日"）——仅当它承载定性结论（"风险收益比在倾斜"）时才随该结论一并进 evidence_quote
