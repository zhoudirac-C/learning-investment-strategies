#!/usr/bin/env python3
"""30min 长周期数据拼接：TDX 分页拉取 → 缓存 /tmp/tdx_60m/{code}.json"""
import sys, os, json
sys.path.insert(0, "/home/ubuntu/learning-investment-strategies/src")
from qing_investment.tdx_market.market import TdxMarket

os.makedirs("/tmp/tdx_60m", exist_ok=True)
m = TdxMarket()

def fetch_30m(code, pages=4, per=800):
    out = []
    for p in range(pages):
        rows = m.get_kline(code, "60min", count=800, start=p*800)
        if not rows:
            break
...
for code in ("sh515980", "sz159381"):
    print(f"== {code} ==")
    data = fetch_30m(code, pages=4)
    json.dump(data, open(f"/tmp/tdx_60m/{code}.json", "w"))
